...
and stored in a directory accessible to Helium. Indeed, every compiler version you install
may have its own version of that configuration file, `hint.conf`.
After installation of Helium, you get an additional program `heliumpath`
that, when run, lists first the directory where the Helium executable is located,
and second, the directory where the `hint.conf` file resides, as well as all
the libraries that come with Helium.
...
We shall now consider all settings except `fontsize`. This is specific to `Hint` and will be discussed only there.
...
If you have paths where `.lvm` files may be found you can add them to `lvmpaths`. If you have more than one,
...
With `logginon` you can specify the standard logging behaviour of the compiler. Use `false` to disable logging, and `true` to enable it. Something similar holds for `overloading`. By changing the setting for `host` and
...
If you have installed `texthint` (you get it as part of `cabal install helium`) on your system, then probably all you need to
...
<pre><your home directory>/.cabal/bin/texthint Hello.hs 
</pre>

As you can see you can also pass along the name of a source file to be compiled and loaded on invocation. You can pass along any number of additional Helium options, but only one file name.

If you prefer not to have to explicitly say where `texthint` is located, add the path to the executable to your `$PATH` environment variable. Consult the manual of the terminal you are working with or the operating system you work under with to see how that may be done. From now on, I shall assume this is exactly what you have done.

`texthint` will load its settings from the `hint.conf` file.  
Contrary to versions of Helium before version 1.7, no other environment settings need to be made.

As explained earlier, you can change the settings by editing the configuration. If you pass additional parameters to `texthint`, then the typical behaviour is that
...
Typically, the interpreter shows you this (it may show a bit more when you run `texthint` for the first
time ever since it will also compile all the modules delivered with Helium):
<pre>               
Compiling ./Hello.hs
