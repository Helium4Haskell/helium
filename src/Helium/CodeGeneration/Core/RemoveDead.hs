...
module Helium.CodeGeneration.Core.RemoveDead
  ( coreRemoveDead,
  )
where
...
usageExpr locals used expr =
  case expr of
    Let binds e ->
      let used' = usageBinds locals used binds
    ...
    Lit _ -> used
    Forall _ e -> usageExpr locals used e
...
usageBinds locals used binds =
  case binds of
    ...
    Rec bs ->
      let (ids, rhss) = unzip $ map (\(Bind (Variable x _) rhs) -> (x, rhs)) bs
...
usageAlt locals used (Alt pat expr) =
  case pat of
    PatCon con _ ids ->
      let locals' = unionSet locals (setFromList ids)
