...
dataDictionary env decla@(UHA.Declaration_Data _ _ (UHA.SimpleType_SimpleType _ _ names) _ _) origin qualname =
  DeclValue
    { declName = name,
      ...
      declType =
        foldr
          (\(typeArg, idx) -> Core.TForall (Core.Quantor idx Core.KStar (Just $ getNameName typeArg)))
  ...
  where
    ...
    dataType =
      Core.typeApplyList (Core.TCon $ Core.typeConFromString $ getNameName qualname) $
    ...
    makeShowDictionary =
      let showBody = dataShowFunction env dictType dataType decla
          ...
          body = foldr (Lam False) (foldl Ap (ApType (Con (ConId $ idFromString "Dict$Prelude.Show") Nothing) dataType) fields) ids
       in foldr
            (\(typeArg, idx) -> Forall (Core.Quantor idx Core.KStar (Just $ getNameName typeArg)))
            body
            (zip names [1 ..])
