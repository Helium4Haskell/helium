...
constructFunctionMap env nrOfSuperclasses name =
  let err = error $ "Invalid class name " ++ show name
...
constructSuperClassMap env name tp =
  let err = error $ "Invalid class name " ++ show name ++ " in " ++ show (classEnvironment env)
...
constructorType typeVar fieldsSuper fieldsMembers classType =
  ...
  where
    ...
    updateTypeVars idxTypeClass a@(Core.TAnn {}) = a
...
classFunctions mod typeOutput className typeVar combinedNames =
  [ DeclCon -- Declare the constructor for the dictionary
      { declName = dictName,
    ...
    ++ map superDict superclasses
    ++ concatMap classFunction combinedNames
  where
    typeArg = Core.TVar 0
...
constructDictionary typeOutput instanceSuperClass combinedNames whereDecls className insName typeVariables' insType origin =
  ...
  where
    ...
    makeBindSuper (cName, _, label, t) =
      let parentMapping :: [(String, String)]
          ...
          resolveSuperInstance (n, var)
            ...
            | (n, fst $ fromJust (find (\x -> snd x == var) parentMapping)) `elem` map (\(a, b) -> (a, getNameName b)) instanceSuperClass && n == cName =
              let Just tvar = find (\(_, cn) -> cn == var) parentMapping
               in Var (idFromString $ "$instanceDict" ++ cName ++ "$" ++ fst tvar)
            | otherwise =
              let -- get all the available super classes
                  repInstanceSuperClass = filter (\(_, v) -> getNameName v == rVar) instanceSuperClass
                  rVar = fst $ fromJust $find (\(_, cn) -> cn == var) parentMapping
                  shortestPath :: [[a]] -> [a]
                  shortestPath [x] = x
                  shortestPath (x : xs) =
                    let sp = shortestPath xs
                  ...
                  constructPath from to
                    | from == to = [[to]]
                    | otherwise =
                      let superClasses = getClassSuperClasses (importEnv typeOutput) from
                          paths :: [[[String]]]
    ...
    makeBindFunc (label, name, t, fdecl) =
      let (_, tp) = declarationType typeOutput (TCCInstance (idFromString insName) insType) name
...
getTVMapping env className insName superClassName =
  let isInsTp :: Tp -> Bool
...
getInstanceSuperClasses env className insName =
  snd $ snd $ fromMaybe (error "Nothing")
    $ find
...
convertDictionaries typeOutput className functions defaults = map makeFunction functions
  where
    ...
    makeFunction fname =
      let fid = idFromString $ constructName fname
