...
eqDict dictType dataType names constructors = foldr
  ...
  where
    ...
    dictBody  = let_
        ...
        (Ap (Ap (ApType (Con $ ConId $ idFromString "Dict$Eq") dataType) (var "default$Eq$/=")) (var "func$eq"))
...
eqFunction dictType dataType typeArgs constructors =
    ...
    in  foldr (Lam False)
              body
              ([Variable (idFromString "dict") dictType, Variable fstArg dataType, Variable sndArg dataType]) -- \$fstArg $sndArg ->
        ...
        [ Alt
            ...
            (if null types
                then Con (ConId (idFromString "True"))
                    ...
                    andCore
                    [ Ap (Ap (Ap (var "==") $ eqFunForType tp) (Var v)) (Var w) | (v, w, tp) <- zip3 vs ws types ]
            )
