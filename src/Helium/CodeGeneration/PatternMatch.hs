...
patternToCore' env types (name, tp, pat) continue nr =
  case pat of
    ...
    Pattern_Variable _ n ->
      withNr nr $
        if name == wildcardId || name == idFromName n
          then continue
          else let_ (idFromName n) (findType n types) (Core.Var name) continue
    -- case _u1 of C _l1 _l2 -> ...
    --             _         -> _next
    Pattern_Constructor range n ps ->
      let (ids, nr') =
            if all isSimple ps
              then (map getIdOfSimplePattern ps, nr)
              else freshIds' "l$" nr (length ps)
    ...
    Pattern_Record range n bs ->
      let ps = rearrangeRecordPatterns env n range bs
    ...
    Pattern_Literal _ l ->
      case l of
        ...
        Literal_Float _ f ->
          withNr nr $
            if_
              (var "$primEqFloat" `app_` float f `app_` Core.Var name)
              continue
