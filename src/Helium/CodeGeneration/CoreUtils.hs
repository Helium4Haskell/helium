...
module Helium.CodeGeneration.CoreUtils
  ( custom,
    ...
    float,
    packedString,
    declarationConstructorType,
    addToTypeEnv,
    declarationType,
    declarationTypeInPattern,
    declarationConstructorTypeScheme,
    toCoreType,
    toCoreTypeNotQuantified,
    typeToCoreType,
    typeToCoreTypeMapped,
    addLambdas,
    ...
    createRecordUpdate,
    createRecordSelector,
    constructorsToCase,
    constructorToCase,
    patternMatchFail,
    patternAlwaysSucceeds,
    getTVar,
    TypeInferenceOutput (TypeInferenceOutput, importEnv),
    lookupBeta,
    setExportsPublic,
  )
where
...
declarationConstructorTypeScheme importEnv name = case M.lookup name $ valueConstructors importEnv of
  Just (_, (Quantification (quantors, qmap, qtp))) ->
    let -- We must assure that the order of the quantors matches the order in which the type variables
...
declarationTypeInPattern typeOutput name beta =
  case declarationTpScheme typeOutput TCCNone name of
    ...
    Nothing ->
      let ty = lookupBeta beta $ typeOutput
...
setExportsPublic implicit (exports, exportCons, exportData, exportDataCon, exportMods) env m =
  ...
  where
    ...
    setPublic decl_
      | isQual decl_ && (isInstance decl_ || declPublic decl_) =
        let name = stringFromId $! declName decl_
            newname = idFromString $! unQualifyString name
         in if not ("Dict" `isPrefixOf` name)
              then [decl_ {declAccess = Export newname}]
              else [decl_]
      | isQual decl_ =
    ...
    isExported decl_ elemIdSet =
      let access = declAccess decl_
       in if implicit
            then case declModule decl_ of
              Just _ -> False -- imported
              Nothing -> True -- defined in current module
            else case declModule decl_ of
              Just x
...
addLambdas typeOutput context beta name args expr = case declarationTpScheme typeOutput context name of
  ...
  Just ty@(Top.Quantification (tvars, qmap, t)) ->
    let -- When a binding is annotated with a type, it may have different type variables in the signature
...
applyTypeSynonym env tp@(Top.TCon name) tps = case M.lookup (nameFromString name) $ typeSynonyms env of
  Just (arity, fn) ->
    let tps' = drop arity tps
        tp' = fn (take arity tps)
     in applyTypeSynonym env tp' tps'
...
createRecordInstantiation typeOutput@TypeInferenceOutput {..} name bindings tps outputTp =
  ...
  where
    ...
    argsWithTypes
      | length args < length sortedFields = zip (map (thd4 . snd) sortedFields) sortedFields
...
constructorsToCase importEnv scrutId resType r tps fs =
  case fs of
    ...
    ((n, fields, exec) : xs) ->
      constructorToCase importEnv n scrutId tps fields exec $
  ...
  Core.ApType (var "$primPatternFailPacked") tp
    `app_` packedString
      ( nodeDescription ++ " ranging from "
          ++ showPosition start
          ++ " to "
          ++ showPosition (getRangeEnd range)
          ++ " in module "
...
coreUndefined importEnv tp =
  foldl
    (\e t -> Core.ApType e $ typeToCoreType t)
    undefinedExpr
  ...
  where
    ...
    undefinedScheme =
      fromMaybe (coreUtilsError "coreUndefined" "undefined not defined") $
