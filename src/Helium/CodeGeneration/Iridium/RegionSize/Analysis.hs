...
analyseInstr envs@(Envs gEnv lEnv) = go   
    where go (Let name expr nextInstr) =  
            let (varAnn, varEff) = analyseExpr  envs expr
                (lEnv2, cont) = analyseInstr envs nextInstr
            in (lEnv2, \bEnv -> let (nxtAnn, nxtEff) = cont bEnv in (nxtAnn, AAdd varEff nxtEff))
          -- ???
          ...
          go (Match local target tpyes fields next) = analyseInstr envs next
