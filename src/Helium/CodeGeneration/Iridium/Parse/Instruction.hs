...
pBind quantors = Bind <$ pToken '%' <*> pId <* pWhitespace <* pToken '=' <* pWhitespace <*> pBindTarget quantors <* pWhitespace <* pToken '$' <* pWhitespace <*> pCallArguments quantors <*> pBindMem

pBindMem :: Parser (Maybe Id)
pBindMem = do
  c <- lookahead
  if c == ';'
    then Just <$ pToken '%' <*> pId <* pChar
    else return Nothing
