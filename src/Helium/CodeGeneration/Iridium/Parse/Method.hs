...
pMethod = do
  ...
  case c of
    '{' ->
      (\(b:bs) -> Method tp' tp' args returnType annotations b bs) <$ pWhitespace <*> pSome (pBlock quantors) pSep
    '=' -> do
      ...
      return $ Method tp' tp' args returnType annotations b []
...
pAbstractMethod = do
  ...
  case c of
    '[' -> do
      ...
      pToken '}'
      pWhitespace
      AbstractMethod (typeRemoveArgumentStrictness tp) (typeRemoveArgumentStrictness tp) fnType <$> pAnnotations
    _ -> do
      pToken ':'
      pWhitespace
      pToken '{'
      ...
      pToken '}'
      pWhitespace
      AbstractMethod sourceType sourceType fnType <$> pAnnotations
