...
module Helium.CodeGeneration.Iridium.Primitive
  ( Primitive (..),
    primitives,
    findPrimitive,
  )
...
import Lvm.Common.Id (Id, NameSupply, idFromString)
import Lvm.Common.IdMap (IdMap, findMap, mapFromList)
...
primCompare name t g = prim' name [Quantor 0 KStar (Just "a")] [t, t, TVar 0, TVar 0] (TVar 0) f
  where
...
compileRead32 target supply [pointer] name =
  [ namePtr LLVM.:= LLVM.BitCast pointer ptrType [],
    ...
    ++ if is32Bit
      then []
      else [name LLVM.:= LLVM.ZExt (LLVM.LocalReference (LLVM.IntegerType 32) nameValue) (LLVM.IntegerType $ fromIntegral $ targetWordSize target) []]
  where
