...
uhaTypetoCoreType m t = case t of
    UHA.Type_Variable    _ n _    -> M.fromJust $ M.lookup n m
    UHA.Type_Constructor _ n _    -> Core.TCon $ typeConstructorToTypeConstant n
...
eqFunForType tpn tse m t = case expandTS tse t of
    UHA.Type_Constructor _ n _    -> var ("$dict" ++ tpn ++ show n)
    ...
    UHA.Type_Variable _ n _ -> var (show n)
...
expandTS tse t@(UHA.Type_Constructor _ n _) = case M.lookup n tse of
