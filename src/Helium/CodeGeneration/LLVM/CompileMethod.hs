...
module Helium.CodeGeneration.LLVM.CompileMethod
  ( compileMethod,
    compileAbstractMethod,
  )
where
...
trampolineBody supply fn params fnType retType = foldr id call instrs
  where
    ...
    call =
      Iridium.Let res (Iridium.Call (Iridium.GlobalFunction fn (length params) fnType) $ map (Right . Iridium.VarLocal) args)
        $ Iridium.Let res' (Iridium.Cast (Iridium.VarLocal $ Iridium.Local res retType) retType')
        $ Iridium.Return
        $ Iridium.VarLocal
        $ Iridium.Local res' retType'
