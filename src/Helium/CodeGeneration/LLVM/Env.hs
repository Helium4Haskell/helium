...
envForModule target mod = Env
  ...
  where
    ...
    methods = fmap (\(Iridium.Declaration name _ _ _ (Iridium.Method _ _ _ _ _ annotations _ _)) -> (name, methodInfo annotations)) (Iridium.moduleMethods mod)
...
methodInfo annotations = EnvMethodInfo (Iridium.callingConvention annotations) (Iridium.MethodAnnotateImplicitIO `elem` annotations)
