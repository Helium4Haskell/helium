...
module Helium.CodeGeneration.LLVM.CompileBlock
  ( compileBlock,
  )
where
...
import LLVM.AST.Constant (Constant (Array, Float, GlobalReference, Int, Undef))
import qualified LLVM.AST.Float as Float
import qualified LLVM.AST.IntegerPredicate as IntegerPredicate
import LLVM.AST.Linkage
import LLVM.AST.Visibility
import Lvm.Common.Id (Id, NameSupply, freshId, idFromString, mapWithSupply, splitNameSupplies, splitNameSupply)
...
compileInstruction env supply (Iridium.Case var (Iridium.CaseConstructor alts))
  ...
  | otherwise =
    let supply1 : supplyCaseInline : supplyCasePointer : _ = splitNameSupplies supply
     ...
     in Partial inlineInstr inlineTerminator [BasicBlock (toName pointerBranch) pointerInstr pointerTerminator]
  where
...
compileExpression env supply expr@(Iridium.Call to@(Iridium.GlobalFunction global arity tp) args) name
  ...
  | not fakeIO =
    let (name', supply') = freshName supply
     ...
     in concat castArgumentInstructions
          ++ [ name'
                 := Call
                   { tailCallKind = Nothing,
                     callingConvention = compileCallingConvention convention,
  ...
  where
    ...
    ioRes = Iridium.DataTypeConstructor ioResId $ Core.typeFromFunctionType $ Core.FunctionType [Left $ Core.Quantor 0 Core.KStar Nothing, Right $ Core.TVar 0, Right Iridium.typeRealWorld] Iridium.typeRealWorld
