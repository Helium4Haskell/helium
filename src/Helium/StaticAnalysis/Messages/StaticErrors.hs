...
import Helium.Syntax.UHA_Utils (convertPredicate, getNameOrigin, removeQualified)
...
data Error
  = NoFunDef Entity Name {-names in scope-} Names
...
instance HasMessage Error where
  getMessage x =
    let (oneliner, hints) = showError x
...
sensiblySimilar name inScope =
  let similars = nub (findSimilar name inScope)
   in if length similars <= 3 -- 3 is the magic number
        then similars
        else []
...
showError anError = case anError of
  NoFunDef TypeSignature name inScope ->
    ( MessageString ("Type signature for " ++ show (show name) ++ " without a definition "),
      [ MessageString ("Did you mean " ++ prettyOrList (map (show . show) xs) ++ " ?")
        | let xs = sensiblySimilar name inScope,
          not (null xs)
      ]
    )
  ...
  ClassMethodContextError Definition className methods ctxt ->
    ( MessageString ("Not allowed to put further restictions on typeClass variable in type class: " ++ (show className) ++ ". "),
      [ MessageString
          ( "In the type signatures of: " ++ prettyAndList (map show methods) ++ " the following context items are not allowed: "
  ...
  InvalidInstanceConstraint instanceName contextName var ->
    ( MessageString
        ( "Context item: " ++ show contextName ++ " with variable: " ++ show var ++ " is not valid for instance of: " ++ show instanceName
            ++ "."
        ),
  ...
  ClassVariableNotInMethodSignature className classVariable methods ->
    ( MessageString ("Class method signature must mention class variable: " ++ show classVariable ++ " in class: " ++ show className ++ "."),
      [ MessageString
          ( "The type signatures of the methods: " ++ prettyAndList (map show methods)
              ++ " must mention type variable: "
              ++ show classVariable
              ++ "."
          )
      ]
  ...
  UndefinedFunctionForClass instanceName name hints ->
    ( MessageString ("Function " ++ show name ++ " not defined in class: " ++ show instanceName ++ "."),
      [ MessageString ("Did you mean " ++ prettyOrList (map (show . show) xs) ++ "?")
        | let xs = sensiblySimilar name hints,
          not (null xs)
      ]
    )
  ...
  Ambiguous entity name names ->
    let showline name'
          | (isImportRange . getNameRange) name' =
            (show . show) name' ++ " imported from module "
              ++ (snd . fromJust . modulesFromImportRange . getNameRange) name'
              ++ " (originally defined in "
              ++ (show . getNameOrigin) name'
              ++ ")"
  ...
  ArityMismatch entity name expected actual ->
    ( MessageString
        ( capitalize (show entity) ++ " " ++ show (show name)
            ++ " should have "
            ++ prettyNumberOfParameters expected
            ++ ", but has "
            ++ if actual == 0 then "none" else show actual
        ),
      []
    )
  ...
  AllFieldsPresent names ->
    ( MessageString
        ( "All fields mentioned should be present in atleast one constructor: "
  ...
  ExportWrongParent entity name parent rightparent rightchilds ->
    let impoorexp = case entity of
          ...
          _ -> "export"
     in ( MessageString $
            (show . show) parent ++ " is not the parent of " ++ show entity
              ++ " "
              ++ (show . show) name
              ++ ". "
              ++ (capitalize . show) entity
          ...
          let childhints = [MessageString $ "Did you mean to " ++ impoorexp ++ " " ++ (show . show) n ++ " with parent " ++ (show . show) parent ++ "?" | n <- sensiblySimilar name rightchilds]
           in if null childhints
                then [MessageString $ "Did you mean to " ++ impoorexp ++ " " ++ (show . show) name ++ " with parent " ++ (show . show) rightparent ++ "?"]
                else childhints
        )
  ExportConflict conflicts ->
    let showline (name', (_, exportString))
          | (isImportRange . getNameRange) name' =
            show exportString ++ " exports: " ++ (show . show) name' ++ " imported from module "
              ++ (snd . fromJust . modulesFromImportRange . getNameRange) name'
              ++ " (orignally defined in "
              ++ (show . getNameOrigin) name'
              ++ ")"
  ...
  NotExportedByModule name importMod inScope ->
    let hints =
     ...
     in (MessageString ("The module " ++ (show . show) importMod ++ " doesn't export " ++ (show . show) name), hints)
  CircularImport names -> (MessageString ("Circular import chain: \n\t" ++ showImportChain (map show names)), [])
  UnknownModule name chain paths ->
    ( MessageString $
        "Can't find module '" ++ show name ++ "'\n"
          ++ "Import chain: \n\t"
...
undefinedConstructorInExpr name sims tyconNames =
  let hints =
        [ "Type constructor " ++ show (show name) ++ " cannot be used in an expression"
...
undefinedConstructorInPat lhsPattern name sims tyconNames =
  let hints =
        [ "Use identifiers starting with a lower case letter to define a function or a variable"
          | lhsPattern
        ]
    ...
    code entity =
      fromMaybe "??"
        ...
        $ [ (TypeSignature, "ts"),
            ...
            (ExportVariable, "ev"),
            (ExportModule, "em"),
            (ExportConstructor, "ec"),
            (ExportTypeConstructorOrClass, "et"),
            (Fixity, "fx"),
            (ImportConstructor, "ic"),
            (ImportTypeConstructorOrClass, "it"),
            (ImportVariable, "iv")
          ]
