...
mergeRanges
  ...
  ( Range_Range
    ...
    | startF1 == stopF1 && startF2 == stopF2 && startF1 == startF2 =
      let (startL, startC, stopL, stopC) =
            if startL1 < startL2 || (startL1 == startL2 && startC1 <= startC2)
              then (startL1, startC1, stopL2, stopC2)
              else (startL2, startC2, stopL1, stopC1)
       in Range_Range
...
sortRanges ranges =
  let (xs, ys) = partition isImportRange ranges
   in sort ys ++ xs
