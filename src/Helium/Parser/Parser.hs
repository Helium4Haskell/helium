  ...
  do
    ...
    <|> do
      ...
      return $ \r ->
        Declaration_FunctionBindings
          r
          [ FunctionBinding_FunctionBinding
              r
              (LeftHandSide_Infix lr (Pattern_Variable nr n) o p)
              b
          ]
    <|> do
      lexAT
    ...
    <|> do
      ...
      return $ \r ->
        if null rs
          then Declaration_PatternBinding r (Pattern_Variable nr n) b
          else
            Declaration_FunctionBindings
              r
              [ FunctionBinding_FunctionBinding
                  r
                  (LeftHandSide_Function lr n ps)
                  b
              ]
...
fexp = addRange $
  do
    ...
    if null es
      then return $ \_ -> e
      else return $ \r -> Expression_NormalApplication r e es
...
operatorAsExpression storeRange =
  ( do
      (o, r) <-
        withRange
          ( fmap Left qvarsym <|> fmap Right qconsym
...
unaryMinusPat =
  do
    (n, mr) <-
      withRange
        ( do lexMINDOT; return floatUnaryMinusName
            ...
            return
              ( \r ->
                  Type_Application
                    r
                    False
                    (Type_Constructor rangeArrow (Name_Special rangeArrow [] [] "->"))
                    [left, right] -- !!!Name
              )
    )
    <?> Texts.parserType
