...
data Env key = Env (Map key Ts) (Map key Ts)
    deriving (Show)

empty :: Env key
empty = Env Map.empty Map.empty

singletonGlobal :: Ord key => key -> Ts -> Env key
singletonGlobal key t = insertGlobal key t empty

singletonLocal :: Ord key => key -> Ts -> Env key
singletonLocal key t = insertLocal key t empty

insertGlobal :: Ord key => key -> Ts -> Env key -> Env key
insertGlobal key t (Env global local) = Env (Map.insert key t global) local

insertLocal :: Ord key => key -> Ts -> Env key -> Env key
...
unionGlobal :: Ord key => Map key Ts -> Env key -> Env key
unionGlobal extendGlobal (Env global local) = Env (Map.union extendGlobal global) local

unionLocal :: Ord key => Map key Ts -> Env key -> Env key
...
(uniqueId, env@(Env global local)) |?| (key, err) = case Map.lookup key local of
    Just x -> freshTfromTs uniqueId x
