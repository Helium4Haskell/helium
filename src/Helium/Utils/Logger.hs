...
unwordsQuoted wrds = unwords (map (quote . escape alertESCAPABLES) wrds)
  where quote s = if ' ' `elem` s then "\"" ++ s ++ "\"" else s -- Not efficient, but balanced.
...
normalizeName name =
  let newname = map toLower (filter isAlphaNum name)
  in  if null newname then loggerDEFAULTNAME else newname

-- Escapes all characters from the list escapables
escape :: String -> String -> String
escape _          []       = []
escape escapables (x : xs) = if x `elem` escapables
  then escapeChar : rest
  else rest
  where rest = x : escape escapables xs
...
logInternalError maybeSources = logger "I" maybeSources internalErrorOptions
 where
  internalErrorOptions =
    [ EnableLogging
    , Host loggerINTERNALERRORHOSTNAME
    , Port loggerINTERNALERRORPORTNR
    ]
...
logger logcode maybeSources options =
  ...
  in  when reallyLog $ do
        ...
        sources      <- case maybeSources of
          Nothing                -> return loggerTERMINATOR
          Just (imports, hsFile) -> do
            let allHsFiles = hsFile : imports
        ...
        {- putStr (normalizeName username ++ 
                        ...
                        "\n" ++sources) -}
        let alertLogcode = if hasAlertOption options
              then map toLower logcode
              else map toUpper logcode
        sendLogString
          hostName
          portNumber
...
sendLogString hostName portNr message loggerDEBUGMODE = N.withSocketsDo
  (rec_ 0)
 where
  rec_ i =
    do --installHandler sigPIPE Ignore Nothing
        handle <- connectTo hostName (fromIntegral portNr)
        hSetBuffering handle (BlockBuffering (Just 1024))
        sendToAndFlush handle message loggerDEBUGMODE
      `CE.catch` \exception -> if i + 1 >= loggerTRIES
                   then debug
                     (  "Could not make a connection: no send ("
                     ++ show (exception :: CE.IOException)
                     ++ ")"
                     )
                     loggerDEBUGMODE
                   else do
                     debug
                       (  "Could not make a connection: sleeping ("
                       ++ show exception
                       ++ ")"
                       )
                       loggerDEBUGMODE
...
combinePathAndFile path file = case path of
  "" -> file
...
--  hClose handle
 ...
 where
  ...
  handler j _ = if j + 1 >= loggerTRIES
    then do
