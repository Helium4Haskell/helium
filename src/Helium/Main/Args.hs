...
simplifyOptions ops =
  let revdefops = reverse ops
      modops = case alertMessageFromOptions revdefops of
        ...
        Nothing -> revdefops
   in collectPaths (keepFirst [Overloading, NoOverloading] (keepFirst [EnableLogging, DisableLogging] modops)) [] []
  where
    ...
    keepFirst _ [] = []
    keepFirst fromList (opt : rest) =
      if opt `elem` fromList
        then opt : optionFilter fromList rest
        else opt : keepFirst fromList rest
    optionFilter _ [] = []
    optionFilter fromList (opt : rest) =
      if opt `elem` fromList
        then optionFilter fromList rest
   ...
   in if not (null errors)
        then
          terminateWithMessage
            options
            "Error in invocation: list of parameters is erroneous.\nProblem(s):"
            (map ("  " ++) errors)
        else
          if length arguments > 1
            then terminateWithMessage options ("Error in invocation: only one non-option parameter expected, but found instead:\n" ++ unlines (map ("  " ++) arguments)) []
            else do
